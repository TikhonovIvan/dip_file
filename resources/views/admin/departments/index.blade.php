                    ...
                    @forelse($filteredDepartments as $department)
                        <tr>
                            ...
                            <td class="text-end d-flex justify-content-end gap-1">
                                <a type="button" class="btn btn-success" href="{{ route('departments.edit', $department->id) }}">
                                ...
                                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#exampleModal-{{ $department->id }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body text-start">
                                                Если вы удалить отдел <strong>( {{$department->name}} )</strong> то все пользователи относящиеся к этому отделу будут удалены.
                                                <br>
                                                <br>
                                            ...
                                            <div class="modal-footer">
                                                <form method="post" action="{{ route('departments.destroy', $department->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <span id="countdownText-{{ $department->id }}" class="text-muted small">Ожидайте... 15 сек</span>
                                                    <button class="btn btn-danger" id="confirmDeleteBtn-{{ $department->id }}" disabled>
                                                       Удалить отдел
                                                    </button>
